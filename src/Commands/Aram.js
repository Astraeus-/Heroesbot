      ...
      updateMember(member, role)
        .then(notificationMessage => {
          this.bot.getDMChannel(msg.author.id)
            ...
            .catch(error => Logger.warn(`Could not notify ${member.username} about ${role.name} status`, error))
        })
        .catch(error => Logger.error(`Could not update ${role.name} status of ${member.username}`, error))
    } else {
      this.bot.getDMChannel(msg.author.id)
